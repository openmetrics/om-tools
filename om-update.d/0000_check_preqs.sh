...
function checkPreqs {
    ...
    if test -z "${OM_INSTALL_ENVFILE}" ; then
            log_red "Couldn't find any Openmetrics instance.env file on this system!\n" >&2
            return 1
    fi


    # source in instance.env
    log_green "Found an instance.env file in ${OM_ENVFILE_DIR}!\n"
    ...
    if test -d "${OM_BASE_DIR}" ; then
        ...
        log_red "Failed to locate Openmetrics OM_BASE_DIR!\n"
        return 1
    fi
