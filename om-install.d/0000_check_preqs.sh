...
function checkPreqs {
	...
	if [ $ret = 1 ] ; then
		...
		log_red "Oh snap! This system lacks software needed to install Openmetrics: ${missing_tools[*]}\n"
		while true; do
    ...
    test -d "${OM_BASE_DIR}" &&
    log_red "Apparently there already exists an Openmetrics installation in '${OM_BASE_DIR}'\n" &&
